...
CLIENT_TIMEOUT = float(os.getenv("OPENAI_TIMEOUT", 120))  # seconds

def _try_fix_json(content: str) -> str:
    """Fix truncated or slightly invalid JSON."""
...
def _extract_content(resp):
    """Universal extractor: GPT-5 / raw JSON fallback."""
    try:
...
def ask_bot(user_slots: str, api_key: Optional[str] = None) -> Dict[str, Any]:
    client = _make_client(api_key=api_key)
    logger.info(user_slots)
    logger.info("Running")    
    SYSTEM = """
        You are **GPT-5**, a consultant-level clinical reasoning engine for physicians.
    ...
    while attempt <= MAX_RETRIES:
        try:
            ...
            if not content:
                ...
                if alt:
                    logger.info("Recovered JSON from GPT-5 parsed field.")
                    return alt
